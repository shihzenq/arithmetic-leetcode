package com.shizhenqiang.arithmetic.leetcode.day36;
...
public class MinPathSumTwo {
    ...
    public static void main(String[] args) {
        ...
        MinPathSumTwo minPathSum = new MinPathSumTwo();
        ...
        for (int i = 0; i < arr.length; i++) {
            ...
            for (int j = 0; j < arr[0].length; j++) {
                if (i == 0 && j == 0) continue;
                    ...
                    System.out.print("i==0 情况下结果："+arr[i][j] +"\t");
                } else if (j == 0) {
                    System.out.print("arr[i - 1][j]: "+arr[i - 1][j] +"\t");
                    System.out.print(arr[i][j] +"\t");
                    arr[i][j] = arr[i - 1][j] + arr[i][j];
                    System.out.print(" j==0 情况下结果："+arr[i][j] +"\t");
                } else {
                    System.out.print("arr[i][j - 1]: "+arr[i][j - 1] +"\t");
                    System.out.print("arr[i - 1][j]: "+arr[i - 1][j] +"\t");
                    arr[i][j] = Math.min(arr[i - 1][j], arr[i][j - 1]) + arr[i][j];
                    System.out.print(" j!=0 && i != 0 情况下结果："+arr[i][j] +"\t");
                }
            }
            System.out.println();
        }
